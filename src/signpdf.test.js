...
import fs from 'fs';
import signer, {P12Signer, SignPdfError} from './signpdf';
...
describe('Test signing', () => {
    ...
    it('signs a ready pdf two times', async () => {
        ...
        signedPdfBuffer = await signer.sign(signedPdfBuffer, new P12Signer(secondP12Buffer, {
    ...
    it('signs big PDF twice producing big AcroForm ID on the first time', async () => {
        ...
        pdfBuffer = await signer.sign(pdfBuffer, new P12Signer(secondP12Buffer, {
            passphrase: 'node-signpdf',
        }));
        expect(pdfBuffer instanceof Buffer).toBe(true);
    ...
    it('signs with passphrased certificate', async () => {
        ...
        pdfBuffer = await signer.sign(
            pdfBuffer,
            new P12Signer(p12Buffer, {passphrase: 'node-signpdf'}),
        );
        expect(pdfBuffer instanceof Buffer).toBe(true);
